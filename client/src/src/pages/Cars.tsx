...
export function Cars() {
  ...
  const filteredCars = cars.filter(car => car.plateNumber.toLowerCase().includes(searchTerm.toLowerCase()) || car.type && car.type.toLowerCase().includes(searchTerm.toLowerCase()) || car.model && car.model.toLowerCase().includes(searchTerm.toLowerCase()) || car.mechanicName && car.mechanicName.toLowerCase().includes(searchTerm.toLowerCase()));
  if (isLoading) {
  ...
  return <div className="space-y-6">
      ...
      {cars.length > 0 && <div className="flex items-center bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 transition-colors">
          <Search className="w-5 h-5 text-gray-400 dark:text-gray-500 mr-2" />
          <input type="text" placeholder="Search by plate number, type, model, or mechanic..." className="flex-1 outline-none text-sm bg-transparent text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>}
        ...
        {cars.length === 0 ? <EmptyState icon={CarIcon} title="No cars yet" description="Get started by adding your first vehicle to the system. You can track service records and payments for each car." actionLabel="Add Your First Car" onAction={() => handleOpenModal()} /> : filteredCars.length === 0 ? <EmptyState icon={Search} title="No results found" description={`No cars match "${searchTerm}". Try adjusting your search.`} /> : <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredCars.map(car => <tr key={car._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {car.year && <div className="text-sm text-gray-500 dark:text-gray-400">
                          {car.year}
                        </div>}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button onClick={() => handleOpenModal(car)} className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 mr-4 transition-colors p-2" title="Edit car">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(car._id)} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 transition-colors p-2" title="Delete car">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={currentCar._id ? 'Edit Car' : 'Add New Car'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Plate Number" value={currentCar.plateNumber || ''} onChange={e => setCurrentCar({
          ...currentCar,
          plateNumber: e.target.value.toUpperCase()
        })} placeholder="e.g., ABC-1234" required />
          <Input label="Type" value={currentCar.type || ''} onChange={e => setCurrentCar({
          ...currentCar,
          type: e.target.value
          ...
          <Input label="Driver Phone" type="tel" value={currentCar.driverPhone || ''} onChange={e => setCurrentCar({
          ...currentCar,
          driverPhone: e.target.value
        })} placeholder="e.g., +1234567890" />
          <Input label="Mechanic Name" value={currentCar.mechanicName || ''} onChange={e => setCurrentCar({
          ...currentCar,
